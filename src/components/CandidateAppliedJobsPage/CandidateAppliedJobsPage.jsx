        ...
        <>
            ...
            <section>
                ...
                {appliedJobsList.map(job => {
                    return (
                        <Box key={job.id} sx={{ border: 'solid black 2px', display: "flex",justifyContent: 'space-between' }}>
                                <Box> <h4 onClick={() => {
                                ...
                                <Box><p>  {job.company_name}</p></Box>
                                <Box><p>  {job.logo_path} </p></Box>
                                <Box><p> {job.company_address}</p></Box>
                                <Box> {job.status === "pending" ?
                                    <p>Applied on {new Date(job.time).toLocaleString()}</p>
                                    :
                                    <></>}</Box>
                                <Box>{job.status === "not_shared" ?
                                    <>
                                        <p>Applied on {new Date(job.time).toLocaleString()}</p>
                                        <button>Chat</button>
                                    </>
